...
var currentTrial = 0;
var allTheData = [];
var TrialLengthOfCurrentPatient = 0;
...
.controller("VisionController", ['$scope', '$http', function ($scope, $http) {
  ...
  $scope.PatientDataAcquistion = function(PatientName) {
    $http.get(aPIROOT + "trialApi/" + PatientName + '/' + testDate).
      success(function(incomingData, status, headers, config) {
        ...
        TrialLengthOfCurrentPatient = TrialLength - 2;
        ...
        if (PatientInfo) {
          allTrialsAtOnce(PatientInfo, Array.apply(null, {length: TrialLength})
                          .map(Number.call, Number).slice(1,));};
  ...
  function allTrialsAtOnce(PatientInfo, TrialLength) {
    ...
    for (trial = 0; trial < TrialLength.length; trial++) {
      ...
      success(function(incomingData, status, headers, config) {
        ...
        for (var i in incomingData) {
          if (i == 0) {
            masterLength = Number(incomingData[i]);
          }
          else {
            if (incomingData[i][1] == null) {
              userData.push({X:0, Y:0});
              ...
              userData.push({X:Number(incomingData[i][1]), Y:Number(incomingData[i][2])});
              if (incomingData[i][0] != currentTrial) {
                currentTrial = incomingData[i][0];
              }
            }
          }
        };
        ...
        if (userData.length >= masterLength) {
            if (userData.length >= masterLength) {
                if (userData[8] != null) {
                  //console.log(currentTrial, userData[8]);
                  var boxData = userData[8];
                  var boxResult = quadrantDetermination(currentTrial, boxData);
                  //console.log(boxResult);
                  allTheData.push({Trial: boxResult.Trial, Quadrant: boxResult.Quadrant, Data: userData});
                  //console.log(allTheData);
                  console.log(TrialLengthOfCurrentPatient, allTheData.length);
                  //If all the data, quadrant and trial is loaded this will be true and fire
                  if (TrialLengthOfCurrentPatient == allTheData.length) {
                    angular.forEach(allTheData, function(value, key) {
                      console.log(value, key);
                      dataBindingToGraph(value.Trial, value.Data);
                    });
                    allTheData.forEach( function(i) {
                      $scope.$watch('i', function() {
                        console.log(i.Trial, i.Data);
                        //Sends all the trial and box data to the directive
                        dataBindingToGraph(i.Trial, i.Data);
                      });
                    });
                  };
                  //boxQuadrants.push(currentTrial, {userData[8]});
              };
            }
        };
      }).
  ...
  function quadrantDetermination(currentTrial, boxData) {
    if (boxData) {
      if (boxData.X > 900) {
        if (boxData.Y > 550) {
            boxQuadrants.push({Trial: currentTrial, Quadrant: 1});
            return ({Trial: currentTrial, Quadrant: 1});
        }
        if (boxData.Y < 550) {
            boxQuadrants.push({Trial: currentTrial, Quadrant: 4});
            return ({Trial: currentTrial, Quadrant: 4});
          }
        }
      if (boxData.X  < 900) {
        if (boxData.Y > 550) {
            boxQuadrants.push({Trial: currentTrial, Quadrant: 2});
            return ({Trial: currentTrial, Quadrant: 2});
        }
        if (boxData.Y < 550) {
            boxQuadrants.push({Trial: currentTrial, Quadrant: 3});
            return ({Trial: currentTrial, Quadrant: 3});
        }
      }
    }
  ...
  function dataBindingToGraph(counter, userData) {
